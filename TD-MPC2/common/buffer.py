...
class Buffer():
    ...
    def _init(self, tds):
        ...
        # Decide whether to use CUDA or CPU memory 
        if self._device.type == 'cuda':
            mem_free, _ = torch.cuda.mem_get_info()
            storage_device = 'cuda:0' if 2.5*total_bytes < mem_free else 'cpu'
        else:
            storage_device = 'cpu'
        print(f'Using {storage_device} for storage')
    ...
    def load(self, td):
        ...
        and is more efficient than adding apisodes one by one
        """
        num_new_eps = len(td)
    ...
    def _prepare_batch(self, td: TensorDict):
        ...
        termination = td.get('terminated', None)
