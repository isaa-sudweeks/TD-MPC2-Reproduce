...
class TDMPC2(torch.nn.Module):
    ...
    def _plan(self, obs, t0=False, eval_mode=False, task=None):
        """
        Plan a sequence of actions using the learned world model. This uses MPPI (Model Predictive Path Integral) 
        control to plan a sequence of actions that maximizes the expected reward.
    ...
    def update_pi(self, zs, task):
        ...
        # TODO: I have no idea what the entropy stuff is 
    ...
    def _update(self, obs, action, reward, termination, task=None):
        ...
        zs[0] = z
        consistency_loss = 0 
        # I wonder if this has the same problem the old RNNs used to have where there is gradient blowup

        for t, (_action, _next_z) in enumerate(zip(action.unbind(0), next_z.unbind(0))):
            z = self.model.next(z, _action, task) # Gets the next predicted state given an action 
